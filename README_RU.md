...
- **Система бэкапов** — полные/только БД, cron-расписание, доставка в Telegram, версионное восстановление
- **Миграция переменных** — устаревшие env-переменные удаляются автоматически при `update` (v2.2.0+)

<details>
<summary><b>💾 Резервное копирование и восстановление</b></summary>

```bash
remnawave backup                    # Полный бэкап (сжатый)
remnawave backup --data-only        # Только база данных
remnawave schedule                  # Настроить cron-расписание
remnawave restore --file backup.tar.gz  # Восстановить с safety-бэкапом
remnawave restore --database-only --file db.sql.gz
```

**Проверка версий:** major/minor должны совпадать; различия patch показывают предупреждения.
