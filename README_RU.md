...
| `--tag VERSION` | Запинить версию образа ноды, например `--tag 2.8.0` — чтобы остаться на 2.x, пока панель не обновлена. Только точные версии: плавающих тегов `2`/`3` у ноды нет. Запиненная нода не двигается при `update` |
